...
class DecisionTreeClassifier:
    ...
    def weighted_gini(self, x_l, x_r):
        return (
            (len(x_l) * self.gini_impurity(x_l)) + (len(x_r) * self.gini_impurity(x_r))
        ) / (len(x_l) + len(x_r))
    ...
    def best_split(self, X, y):
        ...
        if self.bagging:
            features = np.random.choice(
                range(X.shape[1]), size=self.n_col, replace=False
            )
        else:
        ...
        for feature in features:
            ...
            for threshold in thrshlds:
                ...
                if gini < best_gini:
                    if (
                        len(y_left) > self.min_sample_leaf
                        and len(y_right) > self.min_sample_leaf
                    ):
                        best_gini = gini
    ...
    def print_tree(self, tree=None, depth=0):
        ...
        feature_name = self.features[tree.feature]
